...
class LogisticRegression(object):
    ...
    def initialize_weights(self, dim, std_dev=1e-2):
        ...
        #############################################################################
        self.params['W'] = np.random.randn(dim).reshape(dim, 1) * std_dev
        self.params['b'] = 0
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################        
    ...
    def predict(self, X):
        ...
        #############################################################################
        value = self.sigmoid(np.dot(X, W) + b)
        prediction = (value >= 0.5).astype(dtype=np.int32)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def binary_cross_entropy(self, probs, labels):
        ...
        dim = probs.shape[0]
        bce = -np.sum(np.dot(np.log(probs).T, labels) + np.dot(np.log(1 - probs).T, (1 - labels))) / dim
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
