...
class LinearRegression(object):
    ...
    def train_analytic(self, X, y):
        ...
        #############################################################################
        # y = A1 x1 + A2 x2 + ... + b => add bias (Feature_transform)
        # In Matrix form:
        # X1 = [x11, x12, ...], X2 = [x21, x22, ...], X = [X1, X2](Two dimension)
        # Y = [y1, y2, ...]
        # W = [A1, A2, ...]
        # so =>  Y = X * W
        # Xt Y = Xt * X * W (We have to make sure X can be inverse)
        # (Xt X) ^ -1 * Xt * Y = W
        XAddBias = self.feature_transform(X)
        XTranspose = np.transpose(XAddBias)
        XtX = np.dot(XTranspose, XAddBias)
        self.params['W'] = np.dot(np.dot(np.linalg.pinv(XtX), XTranspose), y)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
