...
class RegularizedRegression(object):
    ...
    def poly_feature_transform(self,X,poly_order=1):
        ...
        #############################################################################
        # According to poly_order
        for i in range(1, poly_order):
            f_transform = np.concatenate((f_transform, X ** (i + 1)), axis=1)

        # Add B (y = Ax + "B")
        N = f_transform.shape[0]
        one_form = np.ones(N).reshape(N, 1)
        f_transform = np.concatenate((f_transform, one_form), axis=1)
    ...
    def train_analytic(self, X, y, poly_order=1, lambda_reg=0):
        ...
        #############################################################################
        # T = (D * poly_order + 1)
        X_feature = self.poly_feature_transform(X, self.poly_order)                     # num_test * T
        T = X_feature.shape[1]

        # lambda term
        lambda_term = np.identity(T)

        X_inv = np.linalg.pinv(np.dot(X_feature.T, X_feature) + lambda_reg * lambda_term) # T * T
        self.params['W'] = np.dot(np.dot(X_inv, X_feature.T), y)                        # T * 1
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def loss(self, X, y=None, lambda_reg=0.0):
        ...
        #############################################################################
        grads['W'] = np.dot(X.T, difference) / N + lambda_reg * W
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
